...
public class AAPolygon
{
    ...
    public boolean isComplete()
    {
        ...
        if (axis_last != axis_prev) return true;
    ...
    public boolean contains(Point p)
    {
        ...
        for (int i = 1; i <= points.size(); i++)
        {
            ...
            if (p1.x != p2.x) continue; // Only check vertical lines
            if (p.x  >  p1.x) continue; // If point is past the line in positive direction
            if (p.y  <  fastMin(p1.y, p2.y)) continue; // If below the line minimum
            if (p.y  >  fastMax(p1.y, p2.y)) continue; // If above the line maximum
