...
import io.dallen.kingdoms.core.Structures.Types.Farm;
...
public class PlantGrowthHandler implements Listener {
    ...
    private static ArrayList<Material> cropTypes = new ArrayList<Material>(Arrays.asList(new Material[]{Material.CROPS, Material.POTATO, Material.CARROT, Material.NETHER_WARTS}));
        ...
        if (cropTypes.contains(e.getBlock().getType())) {
            ...
            if (grown && (p = Plot.inPlot(e.getBlock().getLocation())) != null && p instanceof Farm) {
                ...
                if (f.isGrowing()) {
                    ...
                    Bukkit.getScheduler().scheduleSyncDelayedTask(KingdomsCore.getPlugin(), new Runnable() {
                        @Override
                        public void run() {
